...
def export_pricings(
    ...
    business_unit = (
        ...
        .join(finance_models.BusinessUnit.bankAccount)
    ...
    for row in rows:
        ...
        if row.payment_id:
            ...
            if pcapi.domain.reimbursement.DigitalThingsReimbursement.is_relevant(None, booking):
                ...
                # There seems some bookings that have been marked as
                # used before the event date. This should not happen.
                # We should probably unset their "dateUsed.
                log(f"Found booking used before event date {row.booking_id}", business_unit_id)
                continue
            else:
    ...
    # log(f"Wrote {len(csv_rows)} pricings to CSV file: {path}", business_unit_id)
    ...
    #     WHERE date_trunc('year', "valueDate") < 2022
    #
    # and:
    #   INSERT INTO pricing_line ("pricingId", amount, category)
